...
double productions_kernel(DOC * a, DOC * b){
    int n_pairs=0;
    int i,j;
    i = j = 0;
...
//    determine_sub_lists(a->forest_vec[i],a->forest_vec[i],intersect,&norm_a);
//    determine_sub_lists(b->forest_vec[j],b->forest_vec[j],intersect,&norm_b);
//    
//    return n_pairs/sqrt(norm_a * norm_b);

//    return n_pairs/sqrt(a->forest_vec[i]->listSize * b->forest_vec[j]->listSize);
//    return n_pairs;
    return n_pairs/sqrt(a->forest_vec[i]->twonorm_PT * b->forest_vec[j]->twonorm_PT);
}
