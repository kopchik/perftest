    ...
    for(j=0; j<d->num_of_trees;j++)
	    if(d->forest_vec[j]->root != NULL){
			...
			if (d->forest_vec[j]->orderedNodeSet != NULL){
                //				for (i=0; i<d->forest_vec[j]->listSize; i++){
                //					free(d->forest_vec[j]->orderedNodeSet[i].sName);
                //				}
				free(d->forest_vec[j]->orderedNodeSet);
			}
//			if (d->forest_vec[j]->orderedNodeSetbyLevel != NULL){
//                //				for (i=0; i<d->forest_vec[j]->listSize; i++){
//                //					free(d->forest_vec[j]->orderedNodeSetbyLevel[i].sName);			
//                //				}
//				free(d->forest_vec[j]->orderedNodeSetbyLevel);
//			}
			free(d->forest_vec[j]);
        }
	free(d->forest_vec);
...
//	
//	if(node->iNoOfChildren>0){
//		printf("(%s level:%d ",node->sName, node->level); 
//		for(i=0;i<node->iNoOfChildren;i++){
//			writeTreeString(node->pChild[i]);
//		}
//		printf(")"); 
//	}
//	else printf("(%s level:%d)",node->sName, node->level);
//}
	...
	*production=0;
	if(N->iNoOfChildren>0){
        ...
        if (N->production == NULL)
            ...
            list[*counter].sName = N->production;
		N->pre_terminal=pre_term;
		(*counter)++;
        //   	   printf("%s\n",production);
	}
...
char* fillStringHash(TreeNode *node){
    int i;
	//char* tmp = (char*)malloc(sizeof(char)*(MAX_PARSE_TREE_LENGTH+1));
	char tmp[MAX_PARSE_TREE_LENGTH];
	
	if(node->iNoOfChildren>0){
		sprintf(tmp,"(%s ",node->sName); 
		for(i=0;i<node->iNoOfChildren;i++)
			strcat(tmp,fillStringHash(node->pChild[i]));
		strcat(tmp,")");
		node->hash_string = strdup(tmp);
		return node->hash_string;
	}
	else {
		sprintf(tmp,"(%s)",node->sName); 
		node->hash_string = strdup(tmp);
...
/*
 unsigned long _fillStringHash(TreeNode *node){
 int i;
 char* tmp[MAX_PRODUCTION_LENGTH];
 ...
 for(i=0;i<node->iNoOfChildren;i++)
 node->hash = (_fillStringHash(node->pChild[i])) << 5 ) ^ _fillStringHash(node->pChild[i]))) ^ (*p);
 node->hash = (_fillStringHash(node->pChild[i])) << 5 ) ^ _fillStringHash(node->pChild[i]))) ^ ")");
 //node->hash_string = strdup(tmp);
 return node->hash;
 }
 else {
 sprintf(tmp,"(%s)",node->sName);
 node->hash = string_hash(tmp);
 return node->hash;
 }
 }
 */

//void incrLevel(TreeNode *node){
//	node->level +=1;
//}

void genHash(TreeNode *node){
	node->hash = string_hash(node->hash_string);
	free(node->hash_string);
}

void mapTree(TreeNode *node, void (*func)(TreeNode *node)){
    int i;
    
	if(node->iNoOfChildren>0){
		func(node);
		for(i=0;i<node->iNoOfChildren;i++)
			mapTree(node->pChild[i], func);
	}
	else
		func(node);
}

//int compareID(const void* item1, const void* item2)
//{
//	const OrderedTreeNode *it1 = (OrderedTreeNode*)item1;
//	const OrderedTreeNode *it2 = (OrderedTreeNode*)item2;
//	
//	return it1->node->level - it2->node->level; 
//}
...
//int assignNodeLevels(TreeNode *node){
//	/*
//	 Traverses the tree assigning a node level to each node (stored in 'node->level' field)
//	 */
//    int i;
//	
//	node->level = 0;
//	if(node->iNoOfChildren>0){
//		for(i=0;i<node->iNoOfChildren;i++)
//			node->level = max(node->level,assignNodeLevels(node->pChild[i]));
//	}
//	else 
//		node->level = 1;
//	return node->level+1;
...
//int assignNodeIDX(TreeNode *node){
//    int i;
//    
//	if(node->iNoOfChildren>0){
//		for(i=0;i<node->iNoOfChildren;i++)
//			node->idx = string_hash(node->sName) + assignNodeIDX(node->pChild[i]);
//	}
//	else 
//		node->idx = string_hash(node->sName);
//    
//	return node->idx;
//}
...
//	tree->orderedNodeSetbyLevel=(OrderedTreeNode *)malloc(sizeof(OrderedTreeNode)*(tree->root->nodeID+1)); //debug
	fillTheNodeList(tree->root, tree->orderedNodeSet, &counter);     
//	fillTheNodeList(tree->root, tree->orderedNodeSetbyLevel, &counter_level); // debug
    ...
    //TO DO: understand why assigning levels to the nodes slows down the things so much
    //FIX: do not use macros in recursive calls
//	assignNodeLevels(tree->root);
//	qsort(tree->orderedNodeSetbyLevel, counter, sizeof(OrderedTreeNode), &compareID); 
	fillStringHash(tree->root);
    
    //	int i;
    //	for(i=0;i<counter;i++){
    //		printf("label: %s level: %d\n",tree->orderedNodeSetbyLevel[i].sName,tree->orderedNodeSetbyLevel[i].node->level);
    //		printf("hash: %s\n", tree->orderedNodeSetbyLevel[i].node->hash_string);
    //	}
    //	fflush(stdout);
    //	printf("\n\n");
    
	mapTree(tree->root, &genHash);
	
    //	for(i=0;i<counter;i++){
    //		printf("label: %s level: %d\n",tree->orderedNodeSetbyLevel[i].sName,tree->orderedNodeSetbyLevel[i].node->level);
    //		printf("hash: %ld\n", tree->orderedNodeSetbyLevel[i].node->hash);
    //	}
    //	fflush(stdout);
    
    //	/* DEBUG */
    //	
    //	/* do topological sort of the tree nodes by nodeID (node->nodeID) */
    //	int i;
    //	printf("\n");
    //	writeTreeString(tree->root);
    //	
    //	printf("\nBefore sorting by level:\n");
    //	for(i=0;i<counter;i++)
    //		printf("prod:%s ID:%d\n",tree->orderedNodeSetbyLevel[i].sName, tree->orderedNodeSetbyLevel[i].node->level);
    //	fflush(stdout);
    //	
    //	printf("\nAfter sorting by level:\n");
    //		//int (*compar)(const void *, const void*) = &compareID;
    //	/* each node gets assigned its height */
    //	assignNodeLevels(tree->root);
    //	qsort(tree->orderedNodeSetbyLevel, counter, sizeof(OrderedTreeNode), &compareID); 
    //	for(i=0;i<counter;i++)
    //		printf("prod:%s level:%d\n",tree->orderedNodeSetbyLevel[i].sName, tree->orderedNodeSetbyLevel[i].node->level);
    //	fflush(stdout);
    //	
    //	printf("\nPrinting unique ids of nodes:\n");
    //	assignNodeIDX(tree->root);
    //	for(i=0;i<counter;i++)
    //		printf("prod:%s idx:%ld\n",tree->orderedNodeSetbyLevel[i].sName, tree->orderedNodeSetbyLevel[i].node->idx);
    //	fflush(stdout);
    //	
    //	/* end DEBUG */
}
